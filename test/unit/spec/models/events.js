...
describe('Model: EventsModel', function () {

    var EventsModel, $httpBackend, $rootScope, APIBaseUrl, EventsRepository, collectionUrl = 'events';

    beforeEach(function () {

        EventsRepository = jasmine.createSpy('EventsRepository');
        EventsRepository.attach = jasmine.createSpy('EventsRepository.attach');

        module('doujinReleaseTracker', function ($provide) {
        $provide.value('EventsRepository', EventsRepository);
        });

        inject(function (_EventsModel_, _$httpBackend_, _$rootScope_, _APIBaseUrl_) {
          EventsModel = _EventsModel_;
    ...
    it('should have the url property set', function() {
      expect(EventsModel.$settings.url).toBe(APIBaseUrl + collectionUrl);
    });
    ...
    describe('$save', function () {
      it('should PUT its data on $save when it has an ID (update existing)', function() {
        ...
        var model = new EventsModel({title: 'New title', id: 5});
      ...
      it('should POST its data on $save if does not have an ID (new)', function() {
        ...
        var model = new EventsModel({title: 'New title'});
      ...
      it('should attach itself to the Repository on save', function() {
        $httpBackend.expectPUT( APIBaseUrl + collectionUrl + '/5', {title:'New title', id:5}).respond(200, {id: 5, title:'New title from server'});
        var model = new EventsModel({title: 'New title', id: 5});
        expect(EventsRepository.attach).not.toHaveBeenCalled();
        var promise = model.$save();
        $httpBackend.flush();
        expect(EventsRepository.attach).toHaveBeenCalledWith(model);
      });
    });
    ...
    describe('$set', function () {
      it('should load instance and override with new data', function() {
        ...
        expect(model instanceof EventsModel).toBeTruthy();
      });
    ...
    describe('$isDirty', function () {
      it('should return false if object is not changed since last save or delete ', function() {
        var model = new EventsModel({id:1});
        expect(model.$isDirty).toBeFalsy();
      });

      it('should not be dirty initially', function() {
        var model = new EventsModel({id:5});
        expect(model.$isDirty).toBeFalsy();
        $rootScope.$digest();
        expect(model.$isDirty).toBeFalsy();
      });
